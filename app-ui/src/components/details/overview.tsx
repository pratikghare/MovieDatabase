                    ...
                    <HorizontalScroll offset={FULL_WIDTH_OFFSET} className='flex space-x-2 bg-transparent scroll-items'>
                        {
                            details.genres.map((genre: Genre) => (
                                <Chip key={genre.id} className='transition-ease text-xxs rounded-md h-6 border-foreground/30 hover:bg-foreground/30 [&>*]:font-semibold' variant='bordered'>{genre.name}</Chip>
                            ))
                        }
                    </HorizontalScroll>
                }
