...
export default function MediaListCard({ media, className, isRounded }: MediaListCardProps) {
    ...
    return (
        <Card isFooterBlurred className={'border-none overflow-visible bg-transparent shadow-none ' + className} radius={isRounded ? 'none' : 'lg'} style={{ minWidth: width, minHeight: isRounded ? width : height }}>
            <Image
                alt={media.name}
                className='object-cover w-fit shadow-sm'
                src={media.thumbnail}
                style={{ width, height: isRounded ? width : height }}
                radius={ isRounded ? 'full' : 'sm' }
            />
            <CardFooter onClick={() => onClick(media)} className={'items-center cursor-pointer hover:underline justify-center min-h-5 flex-col ' + (width > 100 && !isRounded ? classes : 'bg-transparent backdrop-blur-none backdrop-filter-none p-1')} style={{ maxWidth: width }}>
                <p className='text-xxs text-center p-0 font-bold '>{getShortString(media.name)}</p>

                {
                    !!characters.length &&
                    <div className={'flex items-center text-xs bottom-0 overflow-auto scrollbar-hide '} style={{ maxWidth: (width > 100 && !isRounded) ? (width - 20) : width }}>
                        <p className='text-xxs text-nowrap text-center px-0 min-w-1'>(</p>
                        {
                            characters.map((char: string, index: number) => (
                                <div className='flex items-center' key={char+index}>
                                    <p className='text-xxs text-nowrap text-center px-0'>{char}</p>
                                    {(index < characters?.length - 1) && <p  className='text-xxs text-nowrap text-center px-0 min-w-1 mr-1'>, </p>}
                                </div>
                            ))
                        }
                        <p className='text-xxs text-nowrap text-center px-0 min-w-1'>)</p>
